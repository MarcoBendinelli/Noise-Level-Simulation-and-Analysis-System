...
#define PEOPLE_NUMBER 10000
#define VEHICLE_NUMBER 10000
...
int main(int argc, char  *argv[]) {
	...
	float process0_starting_time;
	float process0_ending_time;
	...
	for (int i=0; i<SIMULATION_STEPS; i++){
		...
		MPI_Barrier(MPI_COMM_WORLD);
		process0_starting_time = MPI_Wtime(); 

		//process 0 sums up the results
		if (my_rank == 0) {
			...
			for (int i=0; i<region_width*region_length; i++){
				for (int j=0; j<world_size; j++){
					region_noise[i] = compute_noise_sum(region_noise[i], results[i + j*region_length*region_width]);
				}
			}

			print_result(region_noise, i);
	

		}

		process0_ending_time = MPI_Wtime(); 
